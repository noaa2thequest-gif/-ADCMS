...
const PermissionManager = {
      ...
      <div class="permissions-list">
        ${perms.map(perm => `
          <div class="permission-item">
            ...
            <button class="revoke-btn" onclick="PermissionManager.revokePermission(${userId}, ${perm.id})">✕</button>
          </div>
        `).join('')}
      </div>
    `;
    ...
    try {
      const response = await fetch('/api/grant-permission', {
        ...
        body: JSON.stringify({
          ...
          permissionId: this.selectedPermissionForGrant
        })
      });
  ...
  async confirmRevoke() {
    ...
    try {
      const response = await fetch('/api/revoke-permission', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: this.selectedUserForGrant,
          permissionId: this.selectedPermissionForRevoke
        })
      });
